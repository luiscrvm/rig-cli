...
export class CICDGenerator {
  ...
  async generateGitHubActions(analysis, options) {
    ...
    try {
      ...
      console.log(chalk.green('\n✅ Generated GitHub Actions files:'));
      console.log(chalk.gray('📁 .github/'));
      console.log(chalk.gray('├── workflows/'));
      console.log(chalk.gray('│   ├── ci-cd.yml'));
      console.log(chalk.gray('│   ├── security.yml'));
      console.log(chalk.gray('│   ├── deploy-staging.yml'));
      console.log(chalk.gray('│   └── deploy-production.yml'));
      console.log(chalk.gray('└── dependabot.yml'));
...
jobs:
  ...
  async generateGitLabCI(analysis, options) {
    ...
    try {
      ...
      console.log(chalk.green('\n✅ Generated GitLab CI/CD file:'));
      console.log(chalk.gray('📁 ./'));
      console.log(chalk.gray('└── .gitlab-ci.yml'));
...
deploy_production:
  ...
  async generateAzureDevOps(analysis, options) {
    const pipelineDir = path.join(this.projectRoot, 'azure-pipelines');
    
    const spinner = ora('Generating Azure DevOps pipelines...').start();
    
    try {
      this.ensureDirectoryExists(pipelineDir);
      
      const pipelineContent = this.generateAzurePipelineYAML(analysis);
      
      fs.writeFileSync(path.join(this.projectRoot, 'azure-pipelines.yml'), pipelineContent);
      
      spinner.succeed('Azure DevOps pipeline generated successfully');
      
      console.log(chalk.green('\n✅ Generated Azure DevOps files:'));
      console.log(chalk.gray('📁 azure-pipelines.yml'));
    } catch (error) {
      spinner.fail('Azure DevOps generation failed');
      throw error;
    }
  }

  async generateJenkins(analysis, options) {
    const jenkinsDir = path.join(this.projectRoot, 'jenkins');
    
    const spinner = ora('Generating Jenkins pipeline...').start();
    
    try {
      this.ensureDirectoryExists(jenkinsDir);
      
      const jenkinsfileContent = this.generateJenkinsfile(analysis);
      
      fs.writeFileSync(path.join(this.projectRoot, 'Jenkinsfile'), jenkinsfileContent);
      
      spinner.succeed('Jenkins pipeline generated successfully');
      
      console.log(chalk.green('\n✅ Generated Jenkins files:'));
      console.log(chalk.gray('📁 Jenkinsfile'));
    } catch (error) {
      spinner.fail('Jenkins generation failed');
      throw error;
    }
  }

  generateAzurePipelineYAML(analysis) {
    return `# Azure DevOps Pipeline
# Generated by Rig CLI - DevOps Infrastructure Generator
...
- stage: Build
  ...
  - job: BuildJob
    ...
    - task: NodeTool@0
      ...
      condition: contains('${analysis?.techStack?.join(' ') || ''}', 'Node.js')
  ...
  - deployment: DeployJob
    ...
    strategy:
      runOnce:
        deploy:
          steps:
          - script: echo "Deploy your application here"
  ...
  generateJenkinsfile(analysis) {
    ...
    stages {
        ...
        stage('Setup') {
            steps {
                script {
                    ${analysis?.techStack?.includes('Node.js') ? `
                    sh 'curl -fsSL https://deb.nodesource.com/setup_18.x | sudo -E bash -'
                    sh 'sudo apt-get install -y nodejs'` : '// Add setup for other tech stacks'}
                }
            }
        }
        ...
        stage('Install Dependencies') {
            steps {
                sh '${this.getInstallCommand(analysis)}'
            }
        }
        ...
        stage('Lint') {
            steps {
                script {
                    try {
                        sh '${this.getLintCommand(analysis)}'
                    } catch (Exception e) {
        ...
        stage('Test') {
            steps {
                sh '${this.getTestCommand(analysis)}'
            }
            post {
                always {
        ...
        stage('Security Audit') {
            steps {
                script {
                    try {
                        sh '${this.getSecurityAuditCommand(analysis)}'
                    } catch (Exception e) {
                        currentBuild.result = 'UNSTABLE'
